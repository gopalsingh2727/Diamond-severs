...
module.exports.operatorLogin = async (event) => {
  ...
  try {
    ...
    // Optional: Populate machine for additional fields (like machineName/type)
    const tokenPayload = {
      _id: operator._id,
...
module.exports.updateMachineStatus = async (event) => {
  ...
  try {
    ...
    // Prevent machine from working on multiple orders
    ...
    // Ensure the machine is the first pending/completable
    ...
    if (newStatus === 'in-progress') {
      ...
      // Check if all machines in this step are completed
